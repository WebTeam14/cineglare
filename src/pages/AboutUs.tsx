  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-black text-white">
      ...
      <style>{`
        ...
        .about-grid {
          background-image: linear-gradient(rgba(128,0,0,.08) 1px, transparent 1px), linear-gradient(90deg, rgba(128,0,0,.08) 1px, transparent 1px);
          ...
          <div className="relative mx-auto flex min-h-[88svh] max-w-7xl items-end px-6 pb-14 pt-28 sm:px-8 md:pb-20 lg:px-12">
            <div className="max-w-4xl">
              ...
              <div className="about-reveal mt-9 flex flex-wrap items-center gap-4 [animation-delay:320ms]">
                <a href="#story" className="group inline-flex items-center gap-3 rounded-full bg-[#800000] px-6 py-3.5 text-sm font-semibold text-white transition-all duration-300 hover:-translate-y-1 hover:bg-[#970000] hover:shadow-[0_12px_35px_rgba(128,0,0,.35)]">
                  Discover Our Story
        ...
        <section id="story" className="relative overflow-hidden bg-white py-20 text-black sm:py-24 lg:py-32">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-12 px-6 sm:px-8 lg:grid-cols-[.8fr_1.2fr] lg:gap-20 lg:px-12">
            ...
            <div className="space-y-6 text-base leading-8 text-black/65 sm:text-lg">
              <p>
                <strong className="font-bold text-black">Cineglare</strong> is an ISO 9001:2015 certified company, committed to delivering excellence through a robust Quality Management System. It is a proud subsidiary of Shield Global Group — a diversified conglomerate with a strong presence across Asia, Africa and Europe, operating actively in over 23 countries.
              </p>
              <p>
        ...
        <section className="relative overflow-hidden bg-[#070707] py-20 sm:py-24 lg:py-32">
          ...
          <div className="relative mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              {[
                { number: "01", label: "Our Direction", title: "Vision", text: "Make every brand a phenomenon and every event a masterpiece." },
                { number: "02", label: "What We Do", title: "Mission", text: "Crafting inspiring brand experiences that celebrate excellence." },
              ].map((item) => (
                <article key={item.number} className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[.035] p-7 backdrop-blur-sm transition-all duration-500 hover:-translate-y-2 hover:border-[#800000]/70 hover:bg-[#800000]/[.06] sm:p-10">
                  <div className="absolute -right-10 -top-14 text-[11rem] font-bold leading-none text-[#800000]/[.08] transition-transform duration-700 group-hover:scale-110">{item.number}</div>
                  <div className="relative">
                    <div className="mb-10 flex items-center justify-between">
                      <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#800000] text-sm font-bold">{item.number}</span>
                      <span className="text-[10px] font-bold uppercase tracking-[.25em] text-white/40">{item.label}</span>
                    </div>
                    <h3 className="text-3xl font-bold sm:text-4xl">{item.title}</h3>
                    <div className="mt-7 border-l-2 border-[#800000] pl-6">
                      <p className="text-xl font-medium leading-8 text-white/75 sm:text-2xl">{item.text}</p>
                    </div>
                    <div className="mt-10 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.2em] text-white/35">
            ...
            <div className="mt-16 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[.025] sm:mt-20">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-5">
                {values.map(({ title, description, icon: Icon }) => (
                  <div key={title} className="group border-b border-white/10 p-6 text-center transition-all duration-300 hover:bg-[#800000]/10 sm:border-r sm:last:border-r-0 lg:border-b-0">
          ...
          <div className="relative mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
            ...
            <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {departments.map((dept) => (
                <article key={dept.number} className="group relative overflow-hidden rounded-[1.5rem] border border-black/10 bg-black p-7 text-white transition-all duration-500 hover:-translate-y-2 hover:bg-[#800000] hover:shadow-[0_25px_60px_rgba(128,0,0,.22)] sm:p-8">
                  <div className="absolute -right-4 -top-8 text-8xl font-bold text-white/[.04] transition-transform duration-500 group-hover:scale-110">{dept.number}</div>
                  <div className="relative">
        ...
        <section className="relative overflow-hidden bg-[#080808]">
          <div className="mx-auto grid max-w-7xl lg:grid-cols-2">
            <div className="flex flex-col justify-center px-6 py-14 sm:px-10 lg:px-16 xl:px-24 lg:py-20">
              <div className="mb-4 flex items-center gap-3">
                <span className="h-px w-12 bg-[#800000]" />
                <span className="text-xs font-semibold uppercase tracking-[0.2em] text-[#800000]">How we work</span>
              </div>
              <h2 className="max-w-xl text-3xl font-bold tracking-tight sm:text-4xl md:text-5xl">
                One team.<span className="block text-[#800000]">Many disciplines.</span>
              </h2>
              <p className="mt-5 max-w-lg text-base leading-relaxed text-white/55">
                Strategy, design, production and delivery move as one — so every brand moment lands with clarity and cinematic impact.
              </p>
              <div className="mt-8 grid gap-4 sm:grid-cols-3">
                {["Art", "Strategy", "Execution"].map((title) => (
                  <div key={title} className="rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-5 text-center">
          ...
          <div className="relative mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
            <div className="max-w-3xl">
              <div className="mb-4 flex items-center gap-3">
                ...
                <span className="text-xs font-semibold uppercase tracking-[0.2em] text-white/70">Beyond business</span>
              </div>
              <h2 className="mt-4 text-3xl font-bold tracking-tight sm:text-4xl md:text-5xl">
                Impact that<span className="block text-white/90">extends further.</span>
              </h2>
            </div>
